...
title: "Issuing SQL to Snowflake Using the SQL API"
subtitle: ""
summary: " "
tags: ["Snowflake"]
...
### Create a User That Can Connect Using Key Pair Authentication
...
Get the string from `snowflake_tf_snow_key.pub` and paste it into `RSA_PUBLIC_KEY_HERE` in the following SQL.
...
```sql
snowsql -a <ACCOUNT_NAME> -u sqlapiuser --private-key-path /Users/jimazato/.ssh/snowflake_tf_snow_key.p8
```

### Generate a JWT

As described in the manual below, generate a JSON Web Token (JWT) which serves as the authentication credential for requests.
...
Save as `sql-api-generate-jwt.py`.
...
Specify the account identifier, Snowflake user, and the path to the private key created earlier as arguments.

```sh
python3 sql-api-generate-jwt.py --account=<ACCOUNT_NAME> --user=sqlapiuser --private_key_file_path=/Users/jimazato/.ssh/snowflake_tf_snow_key.p8
```

```
(base) jimazato@CJ2VQ9Y2M1 sqlapi % python3 sql-api-generate-jwt.py --account=<ACCOUNT_NAME> --user=sqlapiuser --private_key_file_path=/Users/jimazato/.ssh/snowflake_tf_snow_key.p8

INFO:__main__:Creating JWTGenerator with arguments
            account : <ACCOUNT_NAME>, user : sqlapiuser, lifetime : 0:59:00, renewal_delay : 0:54:00
...
INFO:__main__:Generated a JWT with the following payload: {'iss': '<ACCOUNT_NAME>.sqlapiuser.SHA256:maG2a3wOxVnX0cGmhc5SRmWpRzzK5JuNjUm98JRGa1k=', 'sub': '<ACCOUNT_NAME>.sqlapiuser', 'iat': 1672210824, 'exp': 1672214364}
JWT:
eyJ0 (omitted) ixwFE-Ww
```

Issue a request with curl. Specify the string output from the JWT step in `Authorization: Bearer`.

```sh
curl -i -X POST \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer <paste JWT value here>" \
    -H "Accept: application/json" \
    -H "User-Agent: myApplicationName/1.0" \
    -H "X-Snowflake-Authorization-Token-Type: KEYPAIR_JWT" \
    -d "{\"statement\": \"SELECT CURRENT_REGION()\"}" \
    "https://<ACCOUNT_NAME>.snowflakecomputing.com/api/v2/statements"
```
