...
MCP Serverは`llms-full.txt`（約1.8MB / 推定50万トークン）を起動時にHTTPで取得しメモリ上に保持する。しかし、この全データがClaude Codeのコンテキストウィンドウに流れるわけではない。

各ツールがClaude Codeに返すデータ量は以下の通り。

| ツール | 返すデータ | コンテキスト消費 |
| ------ | --------- | --------------- |
| `search_posts` | タイトル一覧のみ | 小 |
| `list_posts` | タイトル一覧のみ | 小 |
| `list_tags` | タグと件数の集計 | 小 |
| `blog_stats` | 統計サマリ | 小 |
| `read_post` | 1記事の全文 | 記事による（大） |

`read_post`以外のツールは検索・集計結果のサマリのみを返すため、コンテキストウィンドウへの影響は小さい。1.8MBの全記事データはMCP Serverプロセスのメモリに留まり、Claude Codeには流れない。`read_post`だけは記事全文を返すため、長い記事の場合はコンテキストを圧迫する可能性がある。
